...
def poc(arg, **kwargs):
    ...
    try:
        ...
        if not -1 == data.find('HHIT CVE-2017-7269 Success'):
            result = {
                "name": "IIS 6.0 webdav",  # 插件名称
                "content": "CVE-2017-7269是IIS 6.0中存在的一个栈溢出漏洞，在IIS6.0处理PROPFIND指令的时候，由于对url的长度没有进行有效的长度控制和检查，导致执行memcpy对虚拟路径进行构造的时候，引发栈溢出，该漏洞可以导致远程代码执行。",  # 插件返回内容详情，会造成什么后果。
